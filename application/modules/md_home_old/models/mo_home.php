...
class mo_home extends CI_Model {
	...
	function getlayanan() {
		...
		$vItems = array();
		$q = 'SELECT id_layanan, nama_layanan, id_group_layanan FROM layanan WHERE id_group_layanan NOT IN ('.$GroupLayanan.')';
		$query = $this->db->query($q);
		foreach ($query->result() as $vRow)
		{
	...
	function getdefaultforward() {
		...
		$q = 'SELECT id_layanan, nama_layanan, id_layanan_forward FROM layanan WHERE id_group_layanan = "'.$GroupLayanan.'"';
		$query = $this->db->query($q);
		foreach ($query->result() as $vRow)
		{
		   $vItems = (array) $vRow;
