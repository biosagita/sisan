<?php
class mo_reg_place extends CI_Model {
	public function __construct() {
		parent::__construct();
	}
	function fnRegPlaceCount($pInsuredKeyword) {
		$this->db->select("count(*) as selectCount");
		$this->db->like(array("regslip_insured"=>$pInsuredKeyword));
		$vResult = $this->db->get("tbregplacing")->result();
		if($vResult) {
			return $vResult[0];
		} else {
			return false;
	...
	function fnRegPlaceData($pInsuredKeyword,$pOffset,$pRows,$pSort,$pOrder) {
		...
		foreach($vResult as $vRow):
			$vArrayTemp['placing_regslip_id'] = $vRow->regslip_id;
			$vArrayTemp['placing_regslip_insured'] = $vRow->regslip_insured;
			$vArrayTemp['placing_regslip_date'] = date("d-M-Y",strtotime($vRow->regslip_date));
			$vArrayTemp['placing_regslip_classrisk'] = $vRow->regslip_classrisk;
			$vArrayTemp['placing_segment'] = $vRow->segment;
			$vArrayTemp['placing_regslip_desc'] = $vRow->regslip_desc;
			$vArrayTemp['placing_approve_by'] = $vRow->approve_by;
	...
	function fnRegPlaceCreate($pData) {
		$vData = array(
		...
		$vResult = $this->db->insert('tbregplacing',$vData);
		if($vResult) {
	...
	function fnRegPlaceUpdate($pRegPlaceId,$pData) {
		$vData = array(
			'regslip_insured' => $pData['vInsured'],
			'regslip_date' => $pData['vDate'],
			'regslip_classrisk' => $pData['vClassRisk'],
		...
		$vResult = $this->db->where('regslip_id',$pRegPlaceId)->update('tbregplacing',$vData);
		if($vResult) {
			echo json_encode(array('success'=>true));
		} else {
			echo json_encode(array('msg'=>'Some errors occured.'));
