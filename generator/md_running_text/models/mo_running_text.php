...
class mo_running_text extends CI_Model {
	...
	function fnrunning_textCount() {
		...
		$vResult = $this->db->get(t_running_text)->result();
		if($vResult) {
			return $vResult[0];
		} else {
			return false;
	...
	function fnCreaterunning_text($pData) {
		...
		$vResult = $this->db->insert('t_running_text',$vData);
		if($vResult) {
	...
	function fnrunning_textDelete($pDelrunning_textId) {
		...
		$vResult = $this->db->where('id_running_text',$pDelrunning_textId)->delete('t_running_text');
	...
	function fnUpdaterunning_text($prunning_textId,$pData) {
		...
		$vResult = $this->db->where('id_running_text',$prunning_textId)->update('t_running_text',$vData);
