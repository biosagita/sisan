...
class md_outbox_sms extends MX_Controller {
	...
	function fnqueue_sms()	{
		$this->load->view('vw_queue_sms');
	}
	// ======================================== 'Datagrid User Section'
	function fnqueue_smsData() {
		...
		$vResult=array();
		$vRs=$this->mo_outbox_sms->fnqueue_smsCount($voutbox_smsKeyword);
		$vResult["total"]=$vRs->selectCount;
		$vResult["rows"]=$this->mo_outbox_sms->fnqueue_smsData($voutbox_smsKeyword,$vOffset,$vRows,$vSort,$vOrder);
	...
	function fnbulk_outbox_smsAdd() {
		$this->load->view('bulk_outbox_sms_add_main');
	}
	...
	function fnoutbox_smsCreate() {
		$vData = array(
			...
			'vf_count'=>$this->input->post('f_count'),
	...
	function fnbulkoutbox_smsCreate() {
		$vData = array(
	...
	$this->mo_outbox_sms->fnCreatebulkoutbox_sms($vData);
	}
	...
	function fnoutbox_smsDelete_all() {
		$this->mo_outbox_sms->fnoutbox_smsDelete_all();
	}
