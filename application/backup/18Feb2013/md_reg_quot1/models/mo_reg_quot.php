<?php
class mo_reg_quot extends CI_Model {
	public function __construct() {
		parent::__construct();
	}
	function fnRegQuotCount($pInsuredKeyword) {
		$this->db->select("count(*) as selectCount");
		$this->db->like(array("regslip_insured"=>$pInsuredKeyword));
		$vResult = $this->db->get("tbregquotation")->result();
		if($vResult) {
			return $vResult[0];
		} else {
			return false;
	...
	function fnRegQuotData($pInsuredKeyword,$pOffset,$pRows,$pSort,$pOrder) {
		...
		foreach($vResult as $vRow):
			$vArrayTemp['quotation_regslip_id'] = $vRow->regslip_id;
			$vArrayTemp['quotation_regslip_insured'] = $vRow->regslip_insured;
			$vArrayTemp['quotation_regslip_date'] = date("d-M-Y",strtotime($vRow->regslip_date));
			$vArrayTemp['quotation_regslip_classrisk'] = $vRow->regslip_classrisk;
			$vArrayTemp['quotation_segment'] = $vRow->segment;
			$vArrayTemp['quotation_regslip_desc'] = $vRow->regslip_desc;
			$vArrayTemp['quotation_approve_by'] = $vRow->approve_by;
	...
	function fnRegQuotCreate($pData) {
		$vData = array(
		...
		$vResult = $this->db->insert('tbregquotation',$vData);
		if($vResult) {
	...
	function fnRegQuotUpdate($pRegQuotId,$pData) {
		$vData = array(
			'regslip_insured' => $pData['vInsured'],
			'regslip_date' => $pData['vDate'],
			'regslip_classrisk' => $pData['vClassRisk'],
		...
		$vResult = $this->db->where('regslip_id',$pRegQuotId)->update('tbregquotation',$vData);
		if($vResult) {
			echo json_encode(array('success'=>true));
		} else {
			echo json_encode(array('msg'=>'Some errors occured.'));
