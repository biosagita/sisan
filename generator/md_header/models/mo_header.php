...
class mo_header extends CI_Model {
	...
	function fnheaderCount() {
		...
		$vResult = $this->db->get(t_header)->result();
		if($vResult) {
			return $vResult[0];
		} else {
			return false;
	...
	function fnCreateheader($pData) {
		...
		$vResult = $this->db->insert('t_header',$vData);
		if($vResult) {
	...
	function fnheaderDelete($pDelheaderId) {
		...
		$vResult = $this->db->where('id_header',$pDelheaderId)->delete('t_header');
	...
	function fnUpdateheader($pheaderId,$pData) {
		...
		$vResult = $this->db->where('id_header',$pheaderId)->update('t_header',$vData);
