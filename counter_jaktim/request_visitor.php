...
if($_GET[r]=="request"){
print"<form action='request_visitor.php' method=GET>";
print"<table width=80% align=center>
	<tr><td bgcolor=#efefef colspan=2><font size=5><b>Request Visitor
	<input type=hidden name='r' value='next' >
	
	<tr><td>NO Antrian<td><input type=text name='no_ticket' >

	<tr><td><input type='submit' value='Proses'>				 
	</table></form>";
}

if($_GET[r]=="next"){
  $sql=mysql_query("UPDATE transaksi set  status_transaksi='1',waktu_panggil='$waktu_panggil',id_loket='$_SESSION[loket]' where no_ticket='$_GET[no_ticket]' and id_group_layanan='$_SESSION[id_group_layanan]' and tanggal_transaksi='$tanggal_transaksi' and status_transaksi !='5' ");  

  print"<meta http-equiv='refresh' content='2; url=http:request_visitor.php?r=reg'>";
...
$sql_count_disp_mk3=mysql_fetch_array($sql_count_disp);
    ...
    $sql_reg=mysql_query("Select visitor.id_visitor,pin_barcode,nama_visitor,transaksi.id_layanan from visitor inner join transaksi ON visitor.id_visitor=transaksi.id_visitor where visitor.id_visitor='$sql_count_disp_mk3[id_visitor]' order by id_layanan desc");
...
print"<table width=80% align=center>
	...
	<tr><td>Barcode<td><input type=text readonly=readonly value='$data_reg[pin_barcode]'>
	<tr><td>Nama Visitor<td><input type=text value='$data_reg[nama_visitor]' name=visitor_name>

	<tr><td><input type='submit' value='Simpan' >
	<a href='http:proses.php?proses=recall'  ><input type=button value='Recall'></a>
