<?php
class md_reg_quot extends MX_Controller {
	public function __construct() {
		parent::__construct();
		$this->load->model('mo_reg_quot');
	...
	function fnRegQuot() {
		$this->load->view('vw_reg_quot');
	}
	function fnRegQuotData() {
		...
		if(!$vSort) {
			...
			if($vSort=='quotation_regslip_id') {
				$vSort='regslip_id';
			} else if($vSort=='quotation_regslip_date') {
				$vSort='regslip_id';
			} else if($vSort=='quotation_regslip_insured') {
				$vSort='regslip_insured';
			} else if($vSort=='quotation_segment') {
				$vSort='segment';
			}
		}
		if(!$vOrder) {
		...
		$vResult=array();
		$vRs=$this->mo_reg_quot->fnRegQuotCount($vInsuredKeyword);
		$vResult["total"]=$vRs->selectCount;
		$vResult["rows"]=$this->mo_reg_quot->fnRegQuotData($vInsuredKeyword,$vOffset,$vRows,$vSort,$vOrder);
		echo json_encode($vResult);
	}
	function fnRegQuotAdd() {
		$this->load->view('_add_main');
	}
	function fnRegQuotEdit($pRegQuotation) {
		$vData['vRegQuotation'] = $pRegQuotation;
		$this->load->view('_add_main',$vData);
	}
	function fnRegQuotRow($pId) {
		$this->mo_reg_quot->fnRegQuotRow($pId);
	}
	function fnClassRiskData() {
		...
		$this->mo_reg_quot->fnClassRiskData($vVarQuery);
	}
	function fnSegmentData() {
		...
		$this->mo_reg_quot->fnSegmentData($vVarQuery);
	}
	function fnRegQuotCreate() {
		...
		$this->mo_reg_quot->fnRegQuotCreate($vData);
	}
	function fnRegQuotUpdate($pRegQuotId) {
		$vDate = $this->input->post('fldDate');
		$vTempDate = explode("/",$vDate);
		$vDate = $vTempDate[2]."-".$vTempDate[1]."-".$vTempDate[0];
		$vData = array(
		...
		$this->mo_reg_quot->fnRegQuotUpdate($pRegQuotId,$vData);
	}
	function fnRegQuotPrint($pSegment,$pRisk,$pId,$pDate,$pApprove) {
		if($pSegment=='kosong') {
			$vSegment = 'AXLE';
		} else {
		...
		$filename = $nowDate.'_'.$nowHour.'_QUOTATION_'.$vRegNo.'_'.$nowUser; // pendefinisian nama dokumen yang akan dibentuk
		$PHPWord = $this->word;
		$document = $PHPWord->loadTemplate(FCPATH.'/assets/Quotation.docx');
	...
	function fnCekRiskSegment($pRisk,$pSegment) {
		$vCekRisk = $this->mo_reg_quot->fnCekRisk($pRisk);
		$vRiskCode = $vCekRisk->ClassRisk;
		$vCekSegment = $this->mo_reg_quot->fnCekSegment($pSegment);
